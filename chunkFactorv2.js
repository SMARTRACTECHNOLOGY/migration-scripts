...
const uri = 'mongodb://127.0.0.1:27017/smartcosmos?authSource=admin'

var oldDB = mongoose.createConnection(uri);
...
var qaDB = mongoose.createConnection('mongodb://127.0.0.1:27017/smartcosmos?authSource=admin');
...
oldDB.once('open', async function () {
    ...
    for (let batch of distinctBatchId) {
        ...
        let customerData = await oldDB.collection("customers").findOne({ 'customerId': customerBatchData?.customerId });
        ...
        while (true) {
            ...
            let tagsData = await oldDB.collection("Tags").find({ bId: batchId }).skip(loopCount).limit(chunkSize).toArray();
