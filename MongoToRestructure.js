...
// new prod string url
var mongoDb = mongoose.createConnection('mongodb://127.0.0.1:27017/elasticMigration?authSource=admin?ssl=true&retryWrites=false');
...
let tenantArray = [
    {
        tenantData: {
            'b529be14-2df8-42bd-806d-3a71d5790298': '0b373478-7a4a-4ae4-8206-6cf1675bcf35'
        },
        process: {
            '969459cd-3780-49a6-87f2-0085a18f5a7b': {
                id:'02be8511-7b8d-4dae-a48e-99ec591c0af3',
                input:['']
            },
        ...
        userId: {
            '3cf7ea6f-8245-4f6c-87f2-43ea28b372b8': {
                username: 'admin@123',
                id: "eb96a560-ec70-4702-b6ae-4016362b0678"
            }
        },
        siteData: {
            '1bfd5bf6-f15e-430e-9652-3bd13fa97a73': {
                siteId: 'dcbd43f9-a5c7-443a-addb-aebd459c5958',
                zoneId: '0e1c1c01-033a-43f8-87cf-98f586761b93',

                siteName: 'Site Mumbai',
                zoneName: 'Zone A2'
            },
        },
        deviceId: '3e5d3e43-bf91-4511-88ca-de8c86149f5f',
        deviceName: "Device 1",
        product: {
            "PTSR22SG":"3bcdc1bb-9631-4879-abb9-eed85559d6dc"
...
mongoDb.once('open', async function () {
    for (let tenantData of tenantArray) {
        try {
            ...
            let enablementData = await mongoDb.collection("authentify").find({ "authentify.tenantId": { $eq: tenantId[0] } },).toArray();
            ...
            for (let enbleData of enablementData) {
                ...
                if(sku) {
                    ...
                    product = enbleData.authentify.metadata?.product?.upc
                }
                let barcode =[]
                let qrcode =[]
                let uhf = []
            ...
            uniqueData.forEach((item) => {
                ...
                console.log(duplicatesArray,"sssssssssssssssssssssssss")
            }) 
            // console.log(filteredData.length, filteredData[0])
            let unAssignedTag = await mongoDb.collection("Qa_Authentify_test9999").insertMany(filteredData);
        } catch (err) {
